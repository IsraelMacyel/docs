...
1. The URL of the site you tried to access is not hosted on VirtusCloud. Any subdomain of a site that is not hosted on VirtusCloud and is attempted to be accessed will receive the message "Unauthorized". If you added the "SUBDOMAIN" field after deploying your application, this field will not be read, and the URL will not exist because this field is only read during the deployment. Therefore, you need to remove and redeploy your application with this field correctly configured.

2. If you have recently hosted your site, please wait 1 minute. VirtusCloud is finalizing the configurations for your site. After this period, try accessing it again.
...
3. In VirtusCloud, the default port for websites and APIs is port `80` for HTTP traffic. If your website or API is using a different port, you may encounter this error message.
