...
description: "Discover the constraints and boundaries of the VirtusCloud Platform."
icon: "shield-dog"
---

## Minimum Resource Requirements
Every project hosted on the VirtusCloud Platform has specific minimum resource requirements to ensure optimal performance:
...
VirtusCloud is committed to providing a balance between resource availability and performance, ensuring a seamless experience for all users.
...
## Service Limits
VirtusCloud implements service limits to ensure fair and efficient resource utilization for all users. These limits are categorized as:

### LACK_OF_RAM
If an application exceeds the allocated RAM limit as specified in its [virtuscloud.app config file](../getting-started/config-file), it will be flagged for violating LACK_OF_RAM and shut down to preserve its own and everyone else's safety.

### LACK_OF_CPU
If an application exceeds and stresses its allocated processing limit, it will be flagged for violating LACK_OF_CPU and shut down to preserve its own and everyone else's safety. It is important to remember that VirtusCloud does not allow the hosting of applications that mine cryptocurrencies, for example.

VirtusCloud is dedicated to providing a secure and balanced environment for all users while ensuring compliance with its policies.
