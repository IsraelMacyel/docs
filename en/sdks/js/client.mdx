...
<Info>
  This page documents **`@virtuscloud/api` v4**. If you are upgrading from v3, read the [v3 → v4 migration guide](/en/sdks/js/migrating_to_v4) first.
</Info>
...
<Tabs>
  <Tab title="npm">
    ```bash theme={null}
    npm install @virtuscloud/api
    ```
  </Tab>

  <Tab title="yarn">
    ```bash theme={null}
    yarn add @virtuscloud/api
    ```
  </Tab>

  <Tab title="pnpm">
    ```bash theme={null}
    pnpm add @virtuscloud/api
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="JavaScript (CommonJS)">
    ```javascript theme={null}
    const { VirtusCloudAPI } = require("@virtuscloud/api");
...
`api.user.get()` returns a [`User`](https://github.com/virtuscloudofc/sdk-api-js/blob/main/src/structures/user.ts) instance containing the account details, current plan, owned applications and owned databases.
...
`user.applications` and `user.databases` are [`Collection`](https://github.com/virtuscloudofc/sdk-api-js/blob/main/src/structures/collection.ts) instances (a `Map` subclass). Iterate them like any `Map`:
...
```typescript theme={null}
import { VirtusCloudAPI, VirtusCloudAPIError } from "@virtuscloud/api";
