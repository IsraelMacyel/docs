---
title: "Hosting a Selenium on VirtusCloud"
sidebarTitle: "Web Navigation with Selenium"
description: "This article guides you to automate web navigation using Selenium and Chromium on VirtusCloud."
---
...
service = Service('/usr/bin/chromedriver') # On VirtusCloud, the path to the Chromium driver is '/usr/bin/chromedriver'
...
The `requirements.txt` file is a file that lists all the external libraries needed for your project. You should list all the libraries that your project will use, excluding native libraries like `random` and `datetime`. This file should be included when submitting your application to VirtusCloud.
...
In VirtusCloud, Chromium is installed in `/usr/bin/chromium`, which is standard. Therefore, the Selenium WebDriver can automatically locate it through the specified ChromeDriver. Thus, there's no need to explicitly specify the Chromium path in the code, unless there are specific reasons to do so, such as custom installations or different environment configurations.
  ...
  <Tab title="CLI Upload">
    <Steps>
      <Step title="First Step">
        ...
        don't have it yet, run the following command in your terminal: ```npm install -g @virtuscloud/cli``` If you already have it, we
