...
Creating the configuration file is very simple: just create a file named `virtuscloud.app` or `squarecloud.config` and add the configuration parameters inside it.

<Steps>
  <Step title="Choose extension">
    The configuration file can have two extensions: **.app** or **.config**. You can use `virtuscloud.app` or `squarecloud.config`, whichever you prefer. Tip: use **.app** to help promote Virtus Cloud. 🥰😂
